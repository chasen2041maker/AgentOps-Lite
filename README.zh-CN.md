...
[PyPI](https://pypi.org/project/groundguard-ai/) | [English](https://github.com/chasen2041maker/GroundGuard/blob/main/README.md) | [Discussions](https://github.com/chasen2041maker/GroundGuard/discussions) | 简体中文
...
<img src="https://raw.githubusercontent.com/chasen2041maker/GroundGuard/main/assets/demo.gif" alt="GroundGuard demo：修正前遗漏 required facts 被红色标出，修正后 verified 绿色通过" width="880">
...
## 0.4.0rc1：确定性 Checker

`0.4.0rc1` 增加了只在单次 `FactGate.check(...)` 调用中显式传入的
request-scoped `Checker`。它们不注册为全局状态，也不会调用网络、数据库或 LLM。

本 RC 新增：

- `Checker` 协议和结构化 `Issue`；
- `Fact` 的 `subject`、`as_of`、`observed_at`、`source_field`、`fact_type`
  上下文字段；
- `OrphanNumberChecker` 与 `RelativeFreshnessChecker`；
- 显式启用的 `groundguard.rules.finance_cn`；
- 向后兼容的 `groundguard.report.v1` 扩展：`issues`、
  `hard_issue_count`、`soft_issue_count`。

`severity="hard"` 的 `Issue` 会让报告变为 `passed=false`；
`severity="soft"` 只提供诊断信息，不改变已有的通过/失败结果。

下面完全使用 synthetic 数据。`finance_cn` 必须显式导入，并由调用方提供
市场上下文：
...
report = gate.check(
    ...
    checkers=(OrphanNumberChecker(), PriceDirectionChecker(), PriceLimitChecker()),
    context={
        "finance_cn": {
            "exchange": "SSE",
            "board": "main",
...
`finance_cn` 当前支持 SSE/SZSE 主板、SSE STAR 与 SZSE ChiNext。缺少必填
上下文时会跳过 hard 涨跌停计算，只给出 soft 的上下文不足诊断。BSE、港股、美股
及其他市场不在支持范围内。规则来源、生效日期、0.01 元 `ROUND_HALF_UP`、多
`subject` 不混算及其他边界见[限制说明](https://chasen2041maker.github.io/GroundGuard/limitations/)。

## 安装

GroundGuard 仍处于 pre-alpha 阶段。PyPI 分发包名是 `groundguard-ai`，Python
导入名仍然是 `groundguard`。
...
PR 自动评论示例见 [GitHub Actions PR-comment 示例](https://github.com/chasen2041maker/GroundGuard/blob/main/docs/examples/github-actions/pr-comment.yml)。
...
- [文档首页](https://chasen2041maker.github.io/GroundGuard/)
- [快速上手](https://chasen2041maker.github.io/GroundGuard/getting-started/)
- [CLI 与配置](https://chasen2041maker.github.io/GroundGuard/cli/)
- [接入配方](https://chasen2041maker.github.io/GroundGuard/recipes/)
- [对比说明](https://chasen2041maker.github.io/GroundGuard/comparisons/)
- [生产使用指南](https://chasen2041maker.github.io/GroundGuard/production/)
...
- [架构设计](https://github.com/chasen2041maker/GroundGuard/blob/main/ARCHITECTURE.md)
- [财务报告 demo](https://github.com/chasen2041maker/GroundGuard/tree/main/examples/financial_report_demo)
- [品牌资产](https://github.com/chasen2041maker/GroundGuard/tree/main/assets/brand)
- [冷启动发布素材](https://github.com/chasen2041maker/GroundGuard/tree/main/docs/launch)
- [更新日志](https://github.com/chasen2041maker/GroundGuard/blob/main/CHANGELOG.md)
- [贡献指南](https://github.com/chasen2041maker/GroundGuard/blob/main/CONTRIBUTING.md)
...
较大的改动请先阅读[贡献指南](https://github.com/chasen2041maker/GroundGuard/blob/main/CONTRIBUTING.md)，并开 issue 说明动机和建议 API。

## License

GroundGuard 基于 [MIT License](https://github.com/chasen2041maker/GroundGuard/blob/main/LICENSE) 发布。
