...
当前声明抽取范围是刻意收窄的：只抽取带单位或量级词的数值声明，例如 `823.2 亿元`、`21.5%`、`10.25 亿美元`、`$3.83 billion`、`USD 10.25M`、`1.2M`、`3,830 million dollars`、`21.5 percent`。没有单位的裸数字会被忽略，以减少误报。
...
assertion schema 包含 `pass`、`success`、`score`、`reason`、`namedScores`，并提供顶层 `claims` 列表方便下游 UI 高亮每条声明；完整 GroundGuard 报告放在 `metadata.groundguard`。
