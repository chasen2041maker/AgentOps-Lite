...
class OutputClaim:
    ...
    start: Optional[int] = None      # 原文中的精确起始偏移，用于高亮/剥离
    end: Optional[int] = None        # 原文中的精确结束偏移，用于高亮/剥离
...
4. 输出 `OutputClaim` 列表，`claim_type="numeric"`，并保留精确 `start/end` 偏移；此时 `status` 默认 `unverified`，交给 matcher 处理。
