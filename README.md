...
> 当前状态：pre-alpha。最小 Python 核心闭环已经可运行：事实账本、显式工具事实登记、输出声明抽取、账本核对、Coverage Report、Policy 门禁、CLI JSON 报告、最小 OpenAI/LangChain 适配器和一个可复现 demo。
...
- 核心事实账本：登记、按 key 查询、TTL 失效过滤、JSONL 落盘/加载
- 引用核对器：从生成文本里抽取输出声明，比对账本，输出 Coverage Report
- 必需事实覆盖检查：识别工具已经返回、但最终答案遗漏的关键事实
- `grounded_generate()`：在模型返回后执行 Policy，支持阻断、标记、剥离未核实声明，并可返回 `GroundedResult`
- CLI 集成：从 Ledger JSONL 与 answer 文本生成 JSON Coverage Report
- 最小框架适配器：OpenAI-compatible chat callable wrapper、LangChain-compatible callback handler

## 仍在计划中

- 更完整的框架适配器示例：LangGraph、常见 Agent 框架、原生装饰器用法
- 更完整的 CI 集成：promptfoo/DeepEval 断言兼容格式、PR 注释
...
OpenAI-compatible wrapper 不依赖 OpenAI SDK 类型，只要求传入一个 chat completion `create` callable：

```python
from groundguard.adapters import openai_chat_llm

llm_call = openai_chat_llm(client.chat.completions.create, model="gpt-4.1-mini")
answer = grounded_generate("总结财务表现", llm_call, ledger)
```

LangChain-compatible callback handler 要求你显式提供 `fact_mapper`，把工具输出中哪些字段可核实说清楚：
...
- LangChain / LangGraph 回调适配（已有最小 callback handler，仍需更多接入示例）
- 原生 Python 装饰器 / context manager 用法
- 常见 Agent 框架的接入示例

### Milestone 3：CI 集成

- 输出 JSON 报告，兼容主流评测工具的断言格式（已有基础 JSON CLI）
- GitHub Action：PR 中标注"这次改动是否引入了新的未核实声明"（已有基础 pytest / diff check CI）
